...
def get_shared_i2c_bus(max_retries=5, retry_delay=1):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"[I2C] Shared bus создана (попытка {attempt})")
            break
...
def i2c_recover():
    ...
    ТОЛЬКО перезагрузка модулей — это закрывает все испорченные FD на /dev/i2c-1.
    i2cdetect НЕ сбрасывает зависший busio-дескриптор.

    Возвращает True после успешного восстановления.
    """
    print("[I2C] Перезагрузка модулей I2C (сброс FD)...")
    try:
        ...
        time.sleep(2)
        print("[I2C] Модули I2C перезагружены")
        return True
    except Exception as e:
